...
def get_email_body(payload):
    """Parse the payload to find the email body."""
    if "parts" in payload:
...
def main():
    ...
    if not CLIENT_ID or not CLIENT_SECRET:
        print("Missing environment variables: GMAIL_CLIENT_ID and GMAIL_CLIENT_SECRET must be set.")
        return

    try:
        email_address = 'bibhu@myhiringpartner.ai'
        message_id = '198855678c870b54'
    ...
    except (SecretManagerError, GmailError, gcloud_exceptions.NotFound) as e:
        print(f"\n❌ An error occurred: {e}")
    except Exception as e:
